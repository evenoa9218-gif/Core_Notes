...
# 「신모」·「심모」는 십모(=10모) 를 받아쓰다 흘린 말이다 — 강사는 6·8·10모를 육모·팔모·십모로
# 부르고, 강의 전사에 「68신모」·「20년 심모」가 함께 남아 있다(D:\_romin_stt). 책은 10모로 적는다.
# 여기서 책 표기로 되돌려야 기출 연결이 그 회차(3차 모의)를 실제로 찾아간다.
# 「10회 사례」 = 제10회 변시 — 전사 "이게 이제 10회 사례형으로 나왔어요", 실제로 변시 10회 사례에 응소 시효중단이 나온다.
# 「법전 모의」 = 법전협 모의 — 전사 "법전의 모의시험에 사례형으로".
LABEL_FIXES = [
    (re.compile(r'(\d{2})\s*년?\s*[신심]모'), r'\1년 10모'),
    (re.compile(r'(?<=【)(\d{1,2})회(\s*)(사례|기록|기출)'), r'변\1\2\3'),
    (re.compile(r'법전\s*모의'), '법전협 모의'),
    (re.compile(r'【판례】'), '【判例】'),   # 한글로 쓴 것 3건 — 나머지 225건과 같은 한자 표기로
]
