...
# 기출 표시를 한 가지 표기로 모은다. 강의를 받아쓴 것이라 같은 시험을 여러 이름으로 부른다
# (법전협 모의 = 변시 모의 = 법모, 모두 법전협 모의시험 하나다). 노션 본문은 그대로 두고 여기서만 맞춘다.
#   신모·심모 = 십모(10모)   강사는 6·8·10모를 육모·팔모·십모로 부른다(전사에 「68신모」·「20년 심모」)
#   19.6모    = 19년 6모     책이 쓰는 점 표기를 연도+월로
#   10회 사례 = 변10 사례    전사 "이게 10회 사례형으로 나왔어요"
#   법무·법전협 모의·변모·모의 = 법모
# 표기가 흔들리면 연결기가 그 회차를 못 찾아간다.
LABEL_INNER = [
    (re.compile(r'(\d{2})\s*년?\s*[신심]모'), r'\1년 10모'),
    (re.compile(r'(\d{2})\s*\.\s*(\d{1,2})\s*모'), r'\1년 \2모'),
    (re.compile(r'^(\d{1,2})회(\s*)(사례|기록|기출)'), r'변\1\2\3'),
    (re.compile(r'법무|법전협?\s*모의|변모|모의'), '법모'),
    (re.compile(r'법모(\s*법모)+'), '법모'),
    (re.compile(r'^판례$'), '判例'),
]
